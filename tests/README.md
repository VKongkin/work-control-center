...
Nine suites, 943 checks, run against a live application.
...
| `knowledge_ui_suite.mjs` | 64 | The same two pages in a browser: writing a runbook and getting it back as rendered markdown, finding it by words in any order, saying it still works, and — on Servers — storing a password without it appearing on screen or in the page source, revealing it deliberately, and reading the access log that records both |
